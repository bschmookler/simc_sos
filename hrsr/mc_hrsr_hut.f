...
C I DON'T KNOW WHERE THE VDC MATERIAL/THICKNESS NUMBERS COME FROM.  DO YOU?
...
!	parameter (hdc_thick  = 1.8)
	parameter (hdc_thick  = 1.5)	!made up number, but want 1/2 thickness
...
C Drift chamber 1 is the focal plane, so shift all zpos values by 25cm
	parameter (hdc_1_zpos = -25.0 + 25.0)
...
C Scintillator positions and thicknesses
	...
	parameter (hscin_1x_offset = 0.00)     !up-down offset to hscin_1x.
...
C any calorimeter cuts, so that pathlen is calculated to the back of the spec.
	drift = hcal_4ta_zpos - hscin_2x_zpos
	radw = drift/hair_radlen
	call project(xs,ys,drift,decay_flag,dflag,m2,p,pathlen)
	if(ms_flag) call musc_ext(m2,p,radw,drift,dydzs,dxdzs,ys,xs)
